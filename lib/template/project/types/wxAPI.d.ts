// https://github.com/Emeryao/typed-we-app/blob/master/we-app.d.ts
...
declare class WXWorker {
  onMessage(callback: (res: any) => any): void;
  postMessage(message: any): void;
  terminate(): void;
}

declare var worker: WXWorker;
...
declare namespace WeApp {
  ...
  interface ICmnAPIParam {
    success?:() => (any | void);
    fail?:(err:any) => (any | void);
    complete?:() => (any | void);
  }
  ...
  interface wx {
    ...
    */
    ...
    /**在当前页面显示导航条加载动画 */
    showNavigationBarLoading(param: ICmnAPIParam): void;
    /**隐藏导航条加载动画 */
  ...
  interface SettingParam extends CallbackParam {
    success: (res: {
      authSetting: {
        /**是否授权==用户信息==，对应接口 wx.getUserInfo */
        'scope.userInfo': boolean;
        /**是否授权==地理位置==，对应接口 wx.getLocation, wx.chooseLocation */
        'scope.userLocation': boolean;
        /**是否授权==通讯地址==，对应接口 wx.chooseAddress */
        'scope.address': boolean;
        /**是否授权==发票抬头==，对应接口 wx.chooseInvoiceTitle */
        'scope.invoiceTitle': boolean;
        /**是否授权==获取发票==，对应接口 wx.chooseInvoice */
        'scope.invoice': boolean;
        /**是否授权==微信运动步数==，对应接口 wx.getWeRunData */
        'scope.werun':boolean;
        /**是否授权==录音功能==，对应接口 wx.startRecord */
        'scope.record': boolean;
        /**是否授权==保存到相册== wx.saveImageToPhotosAlbum, wx.saveVideoToPhotosAlbum */
